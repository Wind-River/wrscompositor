...
function webSocketConnect(soc) {
	...
	if ("WebSocket" in window) {
		...
		gSocket.onmessage = function (e) {
                        //console.log('[41monmessage: [0m'+e.data);
			gsoc.receive(e.data);
		};
	}
}
		...
		if(Gevent.type === "methodReply") {
			...
			for(var i = 0; i < calls.length; i++) {
				...
				if(call && (!call.done) &&
					...
					if(Gevent.data !== undefined && call.successCB !== undefined) {
...
GSocket.prototype.subscribe = function(eventlist) {
	var obj = {
		"type" : "method",
		"name": "subscribe",
...
GSocket.prototype.unsubscribe = function(eventlist)
{
	var obj = {
		"type" : "method",
		"name": "unsubscribe",
